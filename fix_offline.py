...
# 1. Update startCSReview to be completely offline and skip AI
    ...
    try {
        ...
        const questions = sample.map(q => {
            ...
            return {
                ...
                answerIndex: 0,
                // store plain text, we will let checkCSAns handle it directly without inline string escaping issues
...
# 2. Update renderCSQuiz
new_renderCSQuiz = """function renderCSQuiz(questions, topic) {
    const container = document.getElementById('fund-content');
    let currentIdx = 0;
    let score = 0;
    
    // Store questions globally so we don't have inline escaping issues
...
# 3. Update Camera in Mock Interview
camera_code = """// Setup Webcam for ALL Simulations!
...
code = re.sub(r"// Setup Webcam.*?videoElem\.style\.display = 'none';\n\s*\}", camera_code, code, flags=re.DOTALL)
